...
$nombre     = null;
$selCte = $_POST['SelCte'] ?? '';
// Variables que podrían usar los includes (evitar undefined)
    ...
    if ($idVentaPost > 0) {
        ...
        if ($res = $mysqli->query($ventaSql)) {
            if ($Reg = $res->fetch_assoc()) {
                ...
                // Status de referencia: primero POST, si no viene usa el de la venta
...
<body onload="localize()"> <!-- Se lanza la funcion de localizacion -->
    ...
    <div class="modal fade" id="Ventana" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <?php
                if ($Ventana === "Ventana1") {
                    require 'html/EmEdoCte.php'; //Ventana que presenta los datos desglosados de el cleinte 
                } elseif ($Ventana === "Ventana2") {
                    require 'html/Emergente_Registrar_Pago.php'; //Ventana que registra un pago
                } elseif ($Ventana === "Ventana4") {
                    require 'html/NvoCliente.php'; //Ventana que crea un nuevo cliente
                }
                ?>
            </div>
        </div>
    ...
    <div class="principal">
        <div class="d-flex align-items-center py-2 pe-3">
            ...
            <form class="BtnSocial m-0" method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
