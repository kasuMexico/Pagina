<?php
		ob_start();
		require '../formato/dompdfMaster/dompdf_config.inc.php';
		...
		$sql = "SELECT * FROM Usuario WHERE IdContact =$id";
			if($result = $mysqli->query($sql)){									// Si obtienes datos
				while($row = $result->fetch_array()){							//retorname los datos en un arreglo
					 $name = $row['Nombre'];
					 $curp = $row['ClaveCurp'];  										//Datos  del usuario
					 $cont = $row['IdContact'];
				}
			}
			// datos del usuario al tener un contacto
			$sql = "SELECT * FROM Contacto WHERE id=$cont";
			if($result = $mysqli->query($sql)){									// Si obtienes datos
				while($row = $result->fetch_array()){							//retorname los datos en un arreglo
					 $email = $row['Mail'];  												//Datos  del usuario con contacto
					 $phone = $row['Telefono'];
				}
			}
						          ...
						          <tr>
									...
									<table class="ba date">
										<tr>
											...
											<td class="mb ct"> <?php echo $curp; ?></td>
										</tr>
										<tr>
...
// asede a la datos y luego ala pagina
